...
function click_add(sitter_email){
	...
	var option="width=600, height=500, left=100, top=50";
	window.open(url,name,option);
	}
...
<div class="topdiv">
<div class="page-top">
<h2 class="top-h">SITTER 상세보기</h2>
</div>
<c:if test="${profileView.sitter_email eq sessionScope.sitter}">
<div>
<a class="newrq" href="/sitter/rq_list?read=0">새로운 요청건이 ${newrq} 건 있습니다.</a>
</div>
</c:if>
</div>

<div class="detaillist">
	<p class="btnc" id="btnn" onclick="location.href='/sitter/profileBoard'">목록으로</p>
...
<div class="id-img">
<div class="p-gr">
<p class="detailtop-p"> SITTER  </p> 
<p class="detailtop-p"> ${profileView.sitter_email} </p>
<c:if test="${profileView.sitter_email eq sessionScope.sitter}">
<button id="btnn" class="subbtn" type="submit">정보수정</button>
</c:if>
</div>
<div class="img-rate">
...
<c:if test="${profileView.sitter_email eq sessionScope.sitter}">
<div class="btnw" id="btnn"onclick="openWin()">이미지수정</div>
</c:if>
</div>
</div>
...
<input style="padding-bottom: 7px;" type="text" id="sitter_phone" name="sitter_phone" value="${profileView.sitter_phone}" readonly/>
</div>
</div>
...
<div class="loc-top">
<div class="info-p" style="margin-top : 9px;">
<p> 활동 지역 </p>
</div>
<div class="loc-info">
...
<p class="btnr" id="btnn" onclick="click_add()">예약 신청하기</p>
</c:if>
</div>
<div class="signpt">
