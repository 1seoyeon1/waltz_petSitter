...
<div class="page-wrap-two">
   ...
   <a class="pageMakerCat" href="list${pageMake.makeSearch(pageMake.startPage - 1)}">이전</a>
   </c:if>
   <c:forEach begin="${pageMake.startPage }" end="${pageMake.endPage }" var="idx">
   <a href="list${pageMake.makeSearch(idx)}" class="page-num">${idx}</a>
   </c:forEach>
   <c:if test="${pageMake.next && pageMake.endPage > 0 }">
   <a class="pageMakerCat" href="categorylist?blog_category=${keyword}&${pageMake.makeSearch(pageMake.endPage + 1)}">다음</a>
   </c:if>
</div>
   </div>
...
$(function(){
   $('#searchBtn').click(function(){
      self.location = "list" + '${pageMake.makeQuery(1)}' + "&searchType=" + "t"+ "&keyword=" + encodeURIComponent($('#keywordInput').val());
      });
});
</script>
</body>
