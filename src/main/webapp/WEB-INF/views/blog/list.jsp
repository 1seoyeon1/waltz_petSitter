...
<h1 class="header"><span style="font-weight:400;">왈츠</span> 블 로 그 </h1>
</div>
</div>
<hr>
<div class="container">
   ...
   <c:if test="${list.blog_memail != null}">
	<span class="blog-meta_date"><c:out value="${ list.blog_memail}"/></span>
	</c:if>
	<c:if test="${list.blog_semail != null}">
		<span class="blog-meta_date"><c:out value="${ list.blog_semail}"/></span>
	</c:if>
   </div>
        <h3><a class="tag" href="/blog/readView?blog_bno=${list.blog_bno}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType }&keyword=${scri.keyword }">
  <c:out value="${list.blog_title}"/></a></h3>
  <div class="blog-content_preview">
  <c:out value="${list.blog_content}"/>
  </div>
    <a class="tag" href="/blog/readView?blog_bno=${list.blog_bno}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType }&keyword=${scri.keyword }">
    read more
    </a>
   </div>
   </div>
...
<div class="page-wrap">
   ...
   <a class="pageMaker" href="list${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a>
   </c:if>
   <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="idx">
   <a href="list${pageMaker.makeSearch(idx)}" class="page-num">${idx}</a>
   </c:forEach>
   <c:if test="${pageMaker.next&& pageMaker.endPage > 0 }">
   <a class="pageMaker" href="list${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a>
   </c:if>
</div>
   </div>
